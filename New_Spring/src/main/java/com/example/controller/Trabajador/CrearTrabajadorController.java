...
public class CrearTrabajadorController {
    ...
    @PostMapping
    public ResponseEntity<?> crearTrabajador(@RequestBody TrabajadorModel nuevoTrabajador) {
        try {
            // Guardar el nuevo trabajador en la base de datos
            TrabajadorModel trabajadorGuardado = trabajadorRepository.save(nuevoTrabajador);
            // Retornar una respuesta exitosa con un mensaje
            return ResponseEntity.ok().body(new MensajeRespuesta("Trabajador creado exitosamente."));
        } catch (Exception e) {
            // Manejar errores y retornar una respuesta adecuada
            return ResponseEntity.status(500).body(new MensajeRespuesta("Ocurrió un error al crear el trabajador."));
        }
    }
